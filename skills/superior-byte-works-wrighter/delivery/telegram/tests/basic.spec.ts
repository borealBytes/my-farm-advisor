...
import type { MediaAsset, MediaRenderContext, RendererOverrides } from "../src/types.js";
...
const createStubRenderers = (): RendererOverrides => ({
  renderMermaid: async (_code: string, context: MediaRenderContext) => {
  ...
  renderMath: async (_expression: string, context: MediaRenderContext) => {
    const filePath = path.join(
      context.outputDir,
